...
- 预览只展示结构化事实摘要，并在同一轮吸收最终偏好，不生成 HTML；摘要之后必须明确提示：「请确认以上信息是否需要修改？如果没有问题，可以输入“生成简历”来生成您的简历。」用户确认并表达生成意图后，前端会自动调用现有 render 接口。
