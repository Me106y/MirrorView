...
- 用户明确说"没有了"或"就这些"时，只有在语义上表示拒绝继续回答当前项目时才跳过；否则继续处理当前轮未回答的问题。
- 每次追问前先检查完整对话历史和已确认状态；用户已经回答的问题视为已解决，不得换一种说法再次追问同一事实。用户补充了多项细节后，应总结并只追问尚未解决且会影响简历准确性的内容。
- 结束一段经历时，将已确认事实整理为一段简洁摘要写入 `step_states.step4.finalized_experiences`，并将 `step_states.step4.active_focus.stage` 设为 `done`。除非 `user_skipped=true` 或 Grill 已完成至少 2 轮，否则不得结束；如果场景、职责、行动或结果仍有缺失，在状态中记录缺失维度。
