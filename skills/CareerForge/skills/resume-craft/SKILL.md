...
进入最终确认时，将预览、最终偏好和生成确认合并处理：不要先发起独立的“确认偏好”问卷，再在下一轮展示预览。用户未提供额外偏好时沿用已确认的模板、语言、照片和默认专业简洁风格。
...
- 预览只展示结构化事实摘要，并在同一轮吸收最终偏好，不生成 HTML；摘要之后只保留一次提示：「请确认以上信息是否需要修改？如果没有问题，可以输入“生成简历”来生成您的简历。」用户确认并表达生成意图后，前端直接调用现有 render 接口，不再返回或展示一遍预览。
...
- `current_step=6` 才处理最终偏好、预览、修改、确认和生成前状态。
