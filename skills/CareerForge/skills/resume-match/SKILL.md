...
- 由本 Skill 结合完整简历、JD、history 和用户最新消息判断是否需要补充、修改或重新分析，不由运行时根据关键词推进。
- 用户可以在分析后自然语言补充技能、证书、经历或 JD 要求；应更新受影响的结论，而不是把分析结果视为不可修改的终态。
- 运行时只负责输入完整性、结构化结果和安全边界，不覆盖合法的分析解释或回复。
