...
### 投了 100 份简历，回复的只有 3 个——您值得更好的方式。

<br>

*我知道您在海投中一次次刷新邮箱，在 Boss 直聘上一个个点「立即沟通」，*
*却换来一片已读不回的沉默。*
*简历石沉大海，面试不知道怎么准备，求职信写了删、删了写。我们都曾经历过。*

*没吃到时代红利的我们，如今可以吃一吃AI红利了——让 AI 帮您搜岗位、改简历、写求职信、模拟面试吧，*
*把您的时间花在真正重要的事上。*
...
一条命令，自动适配您的 AI Agent——**不需要针对每个 Agent 单独安装**。
...
> 基于 [Vercel Skills CLI](https://github.com/vercel-labs/skills)，自动检测您安装了哪些 Agent（Claude Code、Codex、Cursor、Gemini CLI、Windsurf 等 **50+ 种**），一次安装全部搞定。
>
...
# 复制到您的 Agent 的 skills 目录，例如 Claude Code：
...
```
~/.claude/skills/           # 或您的 Agent 对应的 skills 目录
...
```
您：/job-hunt（或"帮我找工作"、"搜一下合适的岗位"）
AI：请提供您的简历，或者告诉我您的职业方向
您：[上传简历]
AI：您想在哪里找工作？
...
岗位搜索默认走公开搜索引擎（Google/Bing），不需要登录任何平台就能搜到结果。但如果您想直接搜 **Boss 直聘、猎聘、拉勾**等需要登录的平台（数据更全、更新更快），有三种方式：

---

#### 方式一：Chrome 浏览器插件联动（推荐，最强大）

这是效果最好的方式——AI 直接在您已经登录好的浏览器里操作，就像有个助手帮您翻页面一样。
...
3. AI 会在您的浏览器里自动搜索、翻页、提取岗位信息
4. 您只需要看着它操作就行，全程不需要手动干预

**进阶用法——批量提取已收藏的岗位：**

如果您平时在 Boss 直聘上收藏过岗位，或者建过搜索分组（比如"产品经理-深圳"），可以直接跟 AI 说：

> "我在 Boss 直聘上收藏了一些岗位，帮我全部提取出来分析一下"

AI 会自动打开您的收藏页面，滚动加载所有岗位（哪怕有几百个），然后按您的简历逐个做匹配筛选。省掉您一条条点开看的时间。

---

#### 方式二：导出 Cookies 给 AI（不需要装插件）

如果您不想装 Chrome 插件，可以把浏览器的登录 cookies 导出给 AI，它用这个临时凭证帮您搜索。
...
**使用步骤：**
1. 跟 AI 说您想找什么方向的工作
2. AI 会给您一批精心组合的**搜索关键词**（通常 10-20 组，覆盖各种叫法）
3. 您自己在 Boss 直聘上搜这些关键词
4. 把感兴趣的 JD **截图或复制文字**发给 AI
5. AI 帮您做匹配度分析 + 写打招呼消息

虽然需要您手动搜索，但 AI 帮您省掉了最费脑的部分——关键词怎么组合、简历和岗位到底匹不匹配、打招呼消息怎么写。
...
```
您：/resume-match（或"帮我分析一下简历和 JD 的匹配度"）
AI：请提供您的简历和目标岗位 JD
您：[上传简历 + 粘贴 JD]
...
```
您：/resume-craft（或"帮我做一份简历"）
AI：您是想从零做一份新简历，还是优化已有简历？
您：从零做
AI：[通过对话收集您的经历、技能、教育等信息]
AI：推荐 3 种适合您的模板风格，您选哪个？
您：第一个
AI：→ 生成完整的 HTML 简历
   → 自动生成 PDF 文件
   → 内置导出按钮，也可以在浏览器中手动导出
...
```
您：/cover-letter（或"帮我写一封求职信"）
AI：您是要写邮件投递的正式求职信，还是招聘软件上的打招呼消息？
您：邮件投递
AI：→ 基于您的简历和 JD，生成 300-500 字的求职信
   → 不是简历复述，而是讲故事、建立连接
   → 可以多轮修改，直到您满意
